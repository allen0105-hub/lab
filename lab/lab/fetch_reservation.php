...
$stmt = $pdo->prepare("
    SELECT 
        r.*, 
        u.name AS user_name, 
        u.classification AS user_class,
        u.section
    FROM reservations r
...
foreach ($reservationsRaw as $r) {
    ...
    // Combine year & section from users table
    $yearSection = ($r['year_level'] ?? '') . ' - ' . ($r['section'] ?? '');
    ...
    if (isset($grid[$r['day_date']][$r['hour']])) {
        $grid[$r['day_date']][$r['hour']]['reservations'][] = [
            ...
            'reason'           => $r['reason'] ?? '',
            'status'           => $st,
            'section'          => $yearSection, // <-- This will populate Year & Section in modal
        ];
    }
}
?>
      ...
      <?php foreach ($hours as $hour): ?>
        <tr>
          ...
          <?php foreach ($weekDates as $date => $dt):
            ...
            $resCount = count($slot['reservations']);
            $classes = [];
            $clickable = false;
            $contentHtml = '';
            ...
            if ($slot['admin_schedule'] !== null) {
                ...
                if ($resCount >= 3) $classes[] = 'fully-booked';
                $clickable = true;
                $contentHtml = $resCount === 1 ? htmlspecialchars($slot['reservations'][0]['user_name']) : $resCount . " reservations";
            }
          ?>
...
<div id="reservationOverlay">
  <div id="reservationForm" role="dialog" aria-modal="true" aria-labelledby="modalTitle">
    <button class="modal-close" onclick="closeReservationModal()" aria-label="Close">×</button>
...
(function() {
  ...
  const inputSection = document.getElementById('section'); // updated field
  ...
  function showReservation(index) {
    ...
    inputSection.value = res.section || ''; // populate Year & Section
